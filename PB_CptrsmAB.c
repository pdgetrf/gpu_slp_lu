...
void GPU_PB_CptrsmAB0( PBTYP_T * TYPE, char * SIDE, char * UPLO, char * DIAG,
                   int M, int N, char * ALPHA, char * A, int IA, int JA,
                   int * DESCA, char * B, int IB, int JB, int * DESCB,
                   char * * C, int * DESCC, int * CFREE );

#ifdef __STDC__
void GPU_PB_CptrsmAB( PBTYP_T * TYPE, char * VARIANT, char * SIDE, char * UPLO,
...
//#define PRINT
#ifdef PRINT
					if (myrow==0 && mycol==0)
						printf ("\n(%d,%d): TRSM M=%d N=%d, i=%d, j=%d, k=%d, IB=%d, JB=%d\n", 
								myrow, mycol, kbb, N, IA, JA, k, IB, JB);
#endif
					GPU_PB_CptrsmAB0( TYPE, SIDE, UPLO, DIAG, kbb, N, talph, WA, 0, 0,
							WAd, B, IB+k, JB, DESCB, &Bptr, DBUFB, &Bfr );
					/*
					...
					{
						/*
						 *  Replicate B(IB+k:IB+k+kbb-1, JB:JB+N-1) over 
						 *  B(IB+k+kbb:IB+M-1, JB:JB+N-1)
						 */
...
#ifdef PRINT
							if (myrow==0 && mycol==0)
								printf ("\n(%d,%d): GEMM M=%d N=%d, K=%d, i=%d, j=%d\n",
										myrow, mycol, Bmp, Bnq0, kbb, IA, JA);
#endif
							gemm( C2F_CHAR( NOTRAN ), C2F_CHAR( NOTRAN ), &Bmp, &Bnq0,
