...
#ifdef __STDC__
void gpu_pdtrsm_( F_CHAR_T SIDE, F_CHAR_T UPLO, F_CHAR_T TRANS, F_CHAR_T DIAG,
...
#ifdef PRINT
		if (myrow==0 && mycol==0)
			printf ("\n(%d,%d): TRSM M=%d N=%d, Ai=%d, Aj=%d, Bi=%d, Bj=%d\n", 
					myrow, mycol, *M, *N, Ai, Aj, Bi, Bj);
#endif
		GPU_PB_CptrsmAB( type, &Var, &SideOp, &UploA, ( notran ? NOTRAN : TRAN ),
